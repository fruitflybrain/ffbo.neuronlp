...
function register_user(user_details){
    register_session.call('ffbo.server_auth.register_user',user_details).then(
	function(res){
	    console.log(res);
	},
	function(err){
...
function register_user() {
	...
	function(res){
	    console.log(res);
	    if(res){
		feedback.innerHTML = "Username already exists.<br>Please select another username";
		feedback.style.color = "red";
		username.value = "";
	    }
	    else{
		...
		register_session.call('ffbo.auth_server.register_user',[user_details]).then(
		    function(res){
			if('success' in res){
			    bootbox.confirm(res['success'], function (result) { if (result) window.location.href = '../';});
			   ...
			   // feedback.style.color = "green";
			    ...
			    feedback.innerHTML = "We Apologize! An unexpected error occered.<br>Please try again later.";
			    feedback.style.color = "red";
			}
		    },
		    function(err){
